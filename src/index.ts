...
const orderModel = new OrderModel(CDN_URL, API_URL);
const cardsModel = new CardsModel(CDN_URL, API_URL);
...
events.on('basket:open', () => {
  basket.renderSumAllProducts(basketModel.getSumAllProducts());  // отобразить сумма всех товаров в корзине
  let i = 0;
...
//orderModel.total = basketModel.getSumAllProducts(); // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
events.on('success:open', () => {
     ...
     const orderData: IOrderLot = {
        payment: formModel.getOrderLot().payment,
        address: formModel.getOrderLot().address,
        phone: formModel.getOrderLot().phone,
        email: formModel.getOrderLot().email,
        total: basketModel.getSumAllProducts(),
        items: basketModel.basketProducts.map((item) => item.id),
      };
      orderModel.postOrderLot(orderData)
    //   orderModel.postOrderLot(formModel.getOrderLot())
    .then((data) => {
